...
export const GooglePlacesAutocomplete = forwardRef((props, ref) => {
  ...
  const _onPress = (rowData) => {
    if (rowData.isPredefinedPlace !== true && props.fetchDetails === true) {
      ...
      request.onreadystatechange = () => {
        ...
        if (request.status === 200) {
          ...
          if (responseJSON.status === "OK") {
            ...
            props.onPress(rowData, details, inputRef.current);
            // }
          } else {
            _disableRowLoaders();
      ...
      props.onPress(predefinedPlace, predefinedPlace, inputRef.current);
    }
  };
    ...
    if (supportedPlatform() && text && text.length >= props.minLength) {
      ...
      request.onreadystatechange = () => {
        ...
        if (request.status === 200) {
          const responseJSON = JSON.parse(request.responseText);
          if ("osm" === props?.type) {
            const dara = responseJSON.map((place) => {
              return {
            ...
            setDataSource(buildRowsFromResults(dara));
          } else {
      ...
      if ("osm" === props?.type) {
        //https://nominatim.openstreetmap.org/search?q=shafipur&limit=5&format=json&addressdetails=1
        request.open(
          "GET",
          `https://nominatim.openstreetmap.org/search?limit=5&format=json&addressdetails=1&q=` +
      ...
      return (
        ...
        /> */}
          <View style={{ paddingVertical: 10 }}>
            {dataSource.map((item, index) => _renderRow(item, index))}
          </View>
        </>
      );
    }
